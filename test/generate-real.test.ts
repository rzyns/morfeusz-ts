...
describe("Generator path", () => {
	...
	it("generates Sm1 and Sm2 disambiguations", () => {
		const forms = m.generate("kot");
		const sm1Nom = forms.find(f => f.lemma === "kot:Sm1" && f.tag === "subst:sg:nom:m1");
		const sm2Nom = forms.find(f => f.lemma === "kot:Sm2" && f.tag === "subst:sg:nom:m2");
	...
	it("generates forms for czytać (verb)", () => {
		...
		const orthTags = new Set(forms.map(f => `${f.orth}/${f.tag}`));
		expect(orthTags.has("czytał/praet:sg:m1.m2.m3:imperf"), `missing czytał`).toBe(true);
		expect(orthTags.has("czytam/fin:sg:pri:imperf"), `missing czytam`).toBe(true);
		expect(orthTags.has("czytając/pcon:imperf"), `missing czytając`).toBe(true);
	});
