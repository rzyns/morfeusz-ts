...
import { DictionariesRepository, MorfeuszProcessorType, Dictionary } from '../src/index.js';
...
describe('DictionariesRepository', () => {
  it('resolves and loads analyzer dict from default paths if present', async () => {
    ...
    for (const name of candidates) {
      const data = await DictionariesRepository.tryToLoadDictionary(name, MorfeuszProcessorType.ANALYZER);
      if (data) { loaded = { name }; break; }
    }
    if (!loaded) {
      // No dicts available in environment; ensure function returns null and exit
      const data = await DictionariesRepository.tryToLoadDictionary('nonexistent', MorfeuszProcessorType.ANALYZER);
