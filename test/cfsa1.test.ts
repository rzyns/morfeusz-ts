...
function buildCFSA1Buffer(): DataView {
  ...
  const fsaBytes = mappingLen + 1 /*state0 hdr*/ + 1 /*trans first*/ + 1 /*offset byte*/ + 1 /*target hdr*/ + 2 /*len*/ + payload.length;
  ...
  // payload bytes
  view.setUint8(off, payload[0]); off += 1;
  view.setUint8(off, payload[1]); off += 1;
  return view;
}
...
describe('CFSA1', () => {
  it('recognizes a transition using short label and relative offset', () => {
    ...
    const fsa = new CFSA1<InterpsGroupsReader>(view, new MorphDeserializer(), () => new InterpsGroupsReader());
