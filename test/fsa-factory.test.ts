...
import { MAGIC_NUMBER, VERSION_NUM_OFFSET, IMPLEMENTATION_NUM_OFFSET, FSA_DATA_SIZE_OFFSET, FSA_DATA_OFFSET } from '../src/core/fsa/const.js';
...
function buildHeaderWithCFSA1(): ArrayBuffer {
  ...
  off += mappingLen;
  // state0 hdr
  view.setUint8(off, 0x01); off += 1;
  // transition first byte (short=1, offsSize=1)
  view.setUint8(off, (1 << 2) | 1); off += 1;
  // offset 0
  view.setUint8(off, 0x00); off += 1;
  // target hdr (accepting)
  view.setUint8(off, 0x80); off += 1;
  // payload length 2
  view.setUint16(off, 2, false); off += 2;
  // payload bytes
  view.setUint8(off, 0xaa); off += 1;
  view.setUint8(off, 0xbb); off += 1;
  return buf;
}
...
describe('Dictionary FSA factory', () => {
  it('creates SimpleFSA for impl=0 and recognizes', () => {
    ...
    const fsa = dict.createFSA(new MorphDeserializer(), () => new InterpsGroupsReader());
  ...
  it('CFSA1 and CFSA2 are supported in factory', () => {
    ...
    const fsa1 = dict1.createFSA(new MorphDeserializer(), () => new InterpsGroupsReader());
    ...
    const fsa2 = dict2.createFSA(new MorphDeserializer(), () => new InterpsGroupsReader());
