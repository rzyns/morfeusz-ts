...
Before the full golden master suite can assert tag *strings*, the `IdResolver` must be
...
const FIXTURES_DIR = new URL("./fixtures/golden/sgjp-a", import.meta.url).pathname;

type GoldenResult = {
  orth: string; lemma: string; tag: string; name: string; labels: string[];
  tagId: number | null; nameId: number | null; labelsId: number | null;
};
