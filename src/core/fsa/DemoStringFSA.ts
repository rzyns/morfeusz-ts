...
class State<T> implements IState<T> {
  constructor(private node: Node, private readonly deser: Deserializer<T>) {}
  private value?: T;
  private valueSize = 0;
  isSink(): boolean { return this.node.children.size === 0 && !this.node.accepting; }
  isAccepting(): boolean { return this.node.accepting; }
  ...
  getValue(): T {
    if (!this.isAccepting() || this.value === undefined) throw new Error("not accepting");
    return this.value;
  }
...
export class DemoStringFSA<T> implements FSA<T> {
  ...
  constructor(entries: Array<{ word: string; payload: Uint8Array }>, private readonly deser: Deserializer<T>) {
    for (const { word, payload } of entries) {
      let node = this.root;
      for (const cp of word.split("").map(ch => ch.codePointAt(0)!)) {
        if (!node.children.has(cp)) node.children.set(cp, { children: new Map(), accepting: false });
  ...
  getInitialState(): IState<T> { return new State<T>(this.root, this.deser); }
}
