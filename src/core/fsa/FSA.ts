...
class CFsa1State<T> implements State<T> {
  ...
  isSink(): boolean { return this.sink; }
  isAccepting(): boolean { return this.accepting; }
  proceedToNext(fsa: FSA<T>, c: number): void { (fsa as CFSA1<T>).proceedToNextInternal(this, c); }
  getValue(): T { if (!this.accepting) throw new Error('not accepting'); return this.value; }
  getValueSize(): number { return this.valueSize; }
  setNext(offset: number): void { this.sink = false; this.accepting = false; this.offset = offset; this.valueSize = 0; }
  setNextAccepting(offset: number, value: T, size: number): void { this.sink = false; this.accepting = true; this.offset = offset; this.value = value; this.valueSize = size; }
...
export class CFSA1<T> implements FSA<T> {
  ...
  constructor(
    ...
    for (let i = 0; i < mapLen; i++) this.label2Short[i] = this.view.getUint8(i);
    this.initialPtr = CFSA1.INITIAL_ARRAY_STATE_OFFSET;
  }
  getInitialState(): State<T> { return new CFsa1State<T>(0); }
  private readStateData(ptr: number): { next: number; accepting: boolean; transitions: number } {
  ...
  private readTransition(ptr: number): { next: number; shortLabel: number; offsetSize: number } {
  ...
  private readOffset(ptr: number, size: number): { next: number; offset: number } {
    switch (size) {
      case 0: return { next: ptr, offset: 0 };
      case 1: return { next: ptr + 1, offset: this.view.getUint8(ptr) };
      case 2: return { next: ptr + 2, offset: this.view.getUint16(ptr, false) };
      case 3: {
      ...
      default: throw new Error(`Invalid offset size: ${size}`);
    }
  }
    ...
    for (let i = 0; i < sd.transitions; i++) {
      ...
      if (t.shortLabel === shortWanted) {
        if (t.shortLabel === 0) {
          ...
          transPtr += 1;
          if (label === (c & 0xff)) { found = true; td_short = t.shortLabel; td_offsetSize = t.offsetSize; break; }
          const ro = this.readOffset(transPtr, t.offsetSize);
          transPtr = ro.next; // skip unmatched
        } else {
          found = true; td_short = t.shortLabel; td_offsetSize = t.offsetSize; break;
        }
      } else {
...
class CFsa2State<T> implements State<T> {
  ...
  isSink(): boolean { return this.sink; }
  isAccepting(): boolean { return this.accepting; }
  proceedToNext(fsa: FSA<T>, c: number): void { (fsa as CFSA2<T>).proceedToNextInternal(this, c); }
  getValue(): T { if (!this.accepting) throw new Error('not accepting'); return this.value; }
  getValueSize(): number { return this.valueSize; }
  setNext(offset: number): void { this.sink = false; this.accepting = false; this.offset = offset; this.valueSize = 0; }
  setNextAccepting(offset: number, value: T, size: number): void { this.sink = false; this.accepting = true; this.offset = offset; this.value = value; this.valueSize = size; }
...
export class CFSA2<T> implements FSA<T> {
  ...
  getInitialState(): State<T> { return new CFsa2State<T>(0); }
  ...
  private readOffset(ptr: number): { next: number; offset: number; accepting: boolean; last: boolean } {
    const first = this.view.getUint8(ptr);
  ...
  proceedToNextInternal(state: CFsa2State<T>, c: number): void {
    ...
    while (true) {
      if (curr >= this.view.byteLength) { state.setSink(); return; }
      ...
      if (label === (c & 0xff)) {
        ...
        if (ro.last) { state.setSink(); return; }
