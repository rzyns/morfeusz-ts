...
export function readCString(view: DataView, offset: number): { value: string; next: number } {
  let i = offset;
  const bytes: number[] = [];
  while (true) {
...
export function slice(view: DataView, offset: number, length: number): DataView {
