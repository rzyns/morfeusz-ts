import type { InterpsGroupsReader, InterpsGroup } from "./InterpsGroupsReader.js";
import type { MorphInterpretation, IdResolver } from "../types.js";
import { CaseHandling } from "../types.js";
import { isOrthOnlyLower, isOrthOnlyTitle, hasCompressedPrefixCut, getPrefixCutLength, isLemmaOnlyLower, isLemmaOnlyTitle } from "./compression.js";
...
export class InterpsGroupsDecoder {
  ...
  decode(orth: string, reader: InterpsGroupsReader, _ids: IdResolver, handling: CaseHandling): MorphInterpretation[] {
    ...
    while (reader.hasNext()) {
      ...
      while (ptr < end) {
        ...
        if (hasCompressedPrefixCut(g.type)) {
          ...
          prefixToCut = dv.getUint8(ptr); ptr += 1;
        }
        if (ptr >= dv.byteLength) break;
        const suffixToCut = dv.getUint8(ptr); ptr += 1;
        // Read C-string suffixToAdd
        const { value: suffixToAdd, next: nextAfterSuffix } = readCString(dv, ptr);
        ptr = nextAfterSuffix;
        // Lemma case pattern: compressed via group.type flags; no explicit bytes when only-lower/title
        // Tags
        if (ptr + 2 > dv.byteLength) break;
        const tag = dv.getUint16(ptr, false); ptr += 2;
        let nameClassifier = 0;
        if (ptr < dv.byteLength) { nameClassifier = dv.getUint8(ptr); ptr += 1; }
        let qualifiers = 0;
        if (ptr + 2 <= dv.byteLength) { qualifiers = dv.getUint16(ptr, false); ptr += 2; }

        // Assemble lemma from orth using cuts and addition
        const safePrefix = Math.min(Math.max(prefixToCut, 0), orth.length);
        const safeSuffixCut = Math.min(Math.max(suffixToCut, 0), Math.max(orth.length - safePrefix, 0));
        const coreEnd = Math.max(orth.length - safeSuffixCut, safePrefix);
        const core = orth.slice(safePrefix, coreEnd);
        let lemma = core + suffixToAdd;
        lemma = caseHelper.applyLemmaCase(lemma, g.type);

        const interp: MorphInterpretation = { startNode: 0, endNode: 0, orth, lemma, tagId: tag, nameId: nameClassifier, labelsId: qualifiers };
        const matchesStrict = caseHelper.orthMatches(g.type, orth, CaseHandling.STRICTLY_CASE_SENSITIVE);
        all.push(interp);
    ...
    if (res.length === 0) {
      res.push({ startNode: 0, endNode: 0, orth, lemma: orth, tagId: 0, nameId: 0, labelsId: 0 });
    }
    return res;
  }
}
