...
export function hasCompressedOrthCasePatterns(byte: number): boolean {
  return (byte & (CompressionFlags.ORTH_ONLY_LOWER | CompressionFlags.ORTH_ONLY_TITLE)) !== 0;
}
...
export function hasCompressedPrefixCut(byte: number): boolean {
  return (byte & CompressionFlags.PREFIX_CUT_MASK) !== CompressionFlags.PREFIX_CUT_MASK;
}
