...
export class Dictionary {
  ...
  createFSA<T>(deserializer: Deserializer<T>, outFactory: () => T): FSA<T> {
    switch (this.header.impl) {
      case 0:
        return new SimpleFSA<T>(this.header.fsaDataView, deserializer, outFactory);
      case 1:
        return new CFSA1<T>(this.header.fsaDataView, deserializer, outFactory);
      case 2:
        return new CFSA2<T>(this.header.fsaDataView, deserializer, outFactory);
      default:
